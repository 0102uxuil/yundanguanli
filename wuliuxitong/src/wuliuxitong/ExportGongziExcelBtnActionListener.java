...
public class ExportGongziExcelBtnActionListener implements ActionListener {

	GongZiMingXiDan gongzimingxidan;
	Workbook wb;
	
	ExportGongziExcelBtnActionListener(GongZiMingXiDan gzmxd){
		this.gongzimingxidan = gzmxd;
	}
	...
	public void actionPerformed(ActionEvent e) {
        ...
        int returnVal = fc.showSaveDialog(this.gongzimingxidan);
        if (returnVal == JFileChooser.APPROVE_OPTION) {
            File file = fc.getSelectedFile();
            //This is where a real application would save the file.
            if(file != null){
            	if(verifyDuplicate(file.getAbsolutePath() + ".xls")){
            		int jop = JOptionPane.showConfirmDialog(this.gongzimingxidan, "该文件已存在是否覆盖？", "提示", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
            		if(jop == 0){
...
//            	JOptionPane.showMessageDialog(this.huodanPanel, "文件名不能为空！", "文件名为空", JOptionPane.PLAIN_MESSAGE);
	...
	private void exportExcel(String absolutepath_without_postfix){
		wb = new HSSFWorkbook();
		Sheet sheet = wb.createSheet("工资单");
		
		Row row = sheet.createRow(0);
		Cell cell = row.createCell(0);
		cell.setCellValue("工资单");
		
		JTable tb= this.gongzimingxidan.gongzimingxidanTb;
